...
app = Flask(name)
app.secret_key = "pizzabot2026"  # Chave de segurança para gerenciar as sessões do chat
...
@app.route("/")
def index():
		session["historico"] = []  # Limpa a memória do chat ao carregar a página inicial
...
def chat():
		...
		# Envia o histórico acumulado e as diretrizes do robô para a Azure
		...
		# Verifica se a resposta retornada pela IA já é o JSON de fechamento do pedido
		try:
		    dados_pedido = json.loads(conteudo)
		    if dados_pedido.get("pedido_completo"):
		        pedido_id = salvar_pedido(dados_pedido)
		        session["historico"] = []  # Limpa o histórico após fechar o pedido
		        return jsonify({
		    ...
		    pass  # Não é um JSON, continua a conversa em formato texto normal
...
if name == "main":
# Configurado na porta 80 (HTTP Padrão) para evitar bloqueios de proxy escolar
